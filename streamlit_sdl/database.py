...
def load_tenor_average_dataframe(csv_path: str | Path | None = None) -> pd.DataFrame:
    ...
    if len(rows) < 5:
        return pd.DataFrame(columns=["maturity_bucket", "published_ytm", "tenor", "filedate"])

    filedate_raw = rows[2][1] if len(rows[2]) > 1 else None
    filedate = _parse_sdl_datetime(pd.Series([filedate_raw])).iloc[0]

    header_row = rows[3]
    data_rows = [row for row in rows[4:] if len(row) >= 2 and any(cell.strip() for cell in row)]
    dataframe = pd.DataFrame(data_rows, columns=header_row[:2])
