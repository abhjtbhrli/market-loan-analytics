...
def render_statewise_comparison_table(
    ...
    with comparison_right:
        ...
        control_col1, control_col2 = st.columns([0.74, 0.26])
        metric_choice = control_col1.segmented_control(
        ...
        if metric_choice == "Tenor-wise yield":
            selected_tenor_bucket = control_col2.selectbox(
                "Tenor bucket",
                tenor_yield_buckets,
...
def render_gsec_trend_chart(chart_df: pd.DataFrame, selected_window: str = "1M") -> None:
    ...
    if selected_window == "1W":
        week_points = base.mark_point(color="#1f5da8", size=60, filled=True)
        week_labels = base.mark_text(
            dy=-10,
            color="#1f5da8",
            fontSize=11,
            fontWeight="bold",
        ).encode(text=alt.Text("price:Q", format=",.2f"))
        chart = (line + week_points + week_labels).properties(height=360)
    else:
