...
def render_reissue_candidates_table(table_df: pd.DataFrame) -> None:
    ...
    if selected_rows:
        ...
        selected_isin = selected_row.get("isin")
        if selected_isin is not None and not pd.isna(selected_isin):
            st.session_state["persisted_reissue_isin"] = selected_isin
        return selected_row

    selected_isin = st.session_state.get("persisted_reissue_isin")
    if selected_isin is None or pd.isna(selected_isin) or "isin" not in table_df.columns:
        return None

    matching_rows = table_df[table_df["isin"].astype("string") == str(selected_isin)]
...
def _render_state_metric_table(
    ...
    display_df["S. No."] = display_df["S. No."].astype(str)
...
def render_statewise_comparison_table(
    ...
    with comparison_right:
        st.markdown("#### Other comparison metrics")
        toggle_col, note_col = st.columns([0.48, 0.52])
        metric_choice = toggle_col.segmented_control(
            "Select another comparison metric",
            options=["Yield", "Bid cover", "Tenor-wise yield"],
            default="Yield",
            key="statewise_metric_choice",
            label_visibility="collapsed",
        )
        tenor_yield_buckets = analytics.get("tenor_yield_buckets", []) if isinstance(analytics, dict) else []
        tenor_yield_by_bucket = analytics.get("tenor_yield_by_bucket", {}) if isinstance(analytics, dict) else {}
        if metric_choice == "Tenor-wise yield":
            selected_tenor_bucket = note_col.selectbox(
                "Tenor bucket",
                tenor_yield_buckets,
                index=0 if tenor_yield_buckets else None,
                key="statewise_tenor_yield_bucket",
            ) if tenor_yield_buckets else None
        elif metric_choice == "Bid cover":
            note_col.markdown(
                "<p style='margin:0.25rem 0 0 0; font-size:0.8rem; line-height:1.25; white-space:normal; word-break:break-word;'><em>Bid cover: Competitive bids received / Competitive bids accepted (a measure of demand)</em></p>",
                unsafe_allow_html=True,
            )
        else:
            note_col.markdown(
                "<p style='margin:0.25rem 0 0 0; font-size:0.8rem; line-height:1.25; white-space:normal; word-break:break-word;'><em>Yield: Average cut-off yield across auctions for the selected months</em></p>",
                unsafe_allow_html=True,
            )
        ...
        elif metric_choice == "Tenor-wise yield":
            _render_state_metric_table(
                "",
                tenor_yield_by_bucket.get(selected_tenor_bucket, pd.DataFrame()),
                current_fy_label,
                previous_fy_label,
                formatter=lambda value: f"{value:,.2f}",
                caption_text=None,
