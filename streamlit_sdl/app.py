...
from streamlit_sdl.config import DB_SCHEMA_VERSION, DEFAULT_STATE
...
def get_connection(_schema_version: str = DB_SCHEMA_VERSION):
...
def render_homepage() -> None:
    ...
    with link_col1:
        st.markdown("**Assam Dashboard**")
        if st.button("View borrowing trends -> Assam Dashboard", key="home_assam", use_container_width=True):
            st.session_state.selected_page = "Assam"
            st.rerun()
        st.markdown("**State-wise Comparison**")
        if st.button("Compare states -> State-wise Comparison", key="home_states", use_container_width=True):
            st.session_state.selected_page = "State-wise comparison"
            st.rerun()
    with link_col2:
        st.markdown("**Reissue Simulator**")
        if st.button("Evaluate reissue vs fresh borrowing -> Reissue Simulator", key="home_reissue", use_container_width=True):
            st.session_state.selected_page = "Reissuances"
            st.rerun()
        st.markdown("**Interest Rate Movements**")
        if st.button("Track market conditions -> Interest Rate Movements", key="home_rates", use_container_width=True):
            st.session_state.selected_page = "Interest Rate Movements"
            st.rerun()

    st.subheader("How this works (at a glance)")
    st.markdown(
        """
        <style>
        .homepage-flow {
        ...
        <div class="homepage-flow">
            <div class="homepage-box">
                <h4>Data sources</h4>
                <p>&bull; RBI auction data (<a href="https://data.rbi.org.in/#/dbie/home" target="_blank">RBI Database on Indian Economy</a>)</p>
                <p>&bull; FBIL SDL Prices and Yields (<a href="https://www.fbil.org.in/#/home" target="_blank">FBIL</a>)</p>
                <p>&bull; Market benchmark rates of 10Y G-Sec (<a href="https://in.investing.com/rates-bonds/india-10-year-bond-yield-historical-data" target="_blank">Investing.com</a>)</p>
            </div>
            <div class="homepage-arrow">&rarr;</div>
            <div class="homepage-box">
                <h4>Processing</h4>
                <p>&bull; Cleaning and standardisation</p>
                <p>&bull; Joining active ISIN market data to borrowing records</p>
                <p>&bull; Tenor-wise market yield mapping</p>
                <p>&bull; Scenario modelling</p>
            </div>
            <div class="homepage-arrow">&rarr;</div>
            <div class="homepage-box">
                <h4>Outputs</h4>
                <p>&bull; Borrowing analytics</p>
                <p>&bull; Reissue simulations</p>
                <p>&bull; Market insights</p>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def main() -> None:
    ...
    if selected_page == "Reissuances":
        ...
        connection = get_connection(DB_SCHEMA_VERSION)
