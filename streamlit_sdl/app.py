...
from streamlit_sdl.charts import (
    ...
    render_reissue_recommendations,
...
PAGE_QUERY_MAP = {page_name: page_key for page_key, page_name in HOMEPAGE_LINK_MAP.items()}
...
def render_app_style() -> None:
    st.markdown(
        ...
        .page-badge {
            ...
            border-radius: 999px;
            background: linear-gradient(90deg, #fff3bf 0%, #ffd6a5 100%);
            color: #7a0c0c;
            font-size: 0.72rem;
            font-weight: 700;
...
def filter_reissue_candidates_by_vintage(
    table_df: pd.DataFrame,
    vintage_option: str,
) -> pd.DataFrame:
    if table_df.empty or "issue_date" not in table_df.columns or vintage_option == "All":
        return table_df

    filtered_df = table_df.copy()
    filtered_df["issue_date"] = pd.to_datetime(filtered_df["issue_date"], errors="coerce")
    if filtered_df["issue_date"].notna().sum() == 0:
        return filtered_df

    reference_date = filtered_df["issue_date"].max()
    one_year_cutoff = reference_date - pd.DateOffset(years=1)
    two_year_cutoff = reference_date - pd.DateOffset(years=2)

    if vintage_option == "1 year":
        return filtered_df[filtered_df["issue_date"] >= one_year_cutoff]
    if vintage_option == "2 years":
        return filtered_df[filtered_df["issue_date"] >= two_year_cutoff]
    if vintage_option == "Earlier":
        return filtered_df[filtered_df["issue_date"] < two_year_cutoff]
    return filtered_df


def get_persisted_reissue_tenor_range(
    slider_min: int,
    slider_max: int,
) -> tuple[int, int]:
    stored_value = st.session_state.get("persisted_reissue_tenor_range", st.session_state.get("reissue_tenor_range"))
    if not isinstance(stored_value, tuple) or len(stored_value) != 2:
        stored_value = (slider_min, slider_max)
    min_value = int(max(slider_min, min(slider_max, stored_value[0])))
    max_value = int(max(min_value, min(slider_max, stored_value[1])))
    return (min_value, max_value)


def get_default_statewise_months() -> list[str]:
    fiscal_months = ["Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec", "Jan", "Feb", "Mar"]
    today = pd.Timestamp.today()
    current_month = int(today.month)
    month_to_fiscal_index = {
        4: 1,
        5: 2,
        6: 3,
        7: 4,
    ...
    cutoff_index = month_to_fiscal_index.get(current_month, 12)
    return fiscal_months[:cutoff_index]
...
def render_reissue_simulation_panel(
    ...
    with st.container():
        ...
        default_reissue_amount = 1000.0

        if st.session_state.get("reissue_amount_seed_isin") != selected_isin:
            st.session_state["reissue_amount_seed_isin"] = selected_isin
            st.session_state["reissue_amount_cr"] = default_reissue_amount
            st.session_state["persisted_reissue_amount_cr"] = default_reissue_amount
        elif "reissue_amount_cr" not in st.session_state:
            st.session_state["reissue_amount_cr"] = float(
                st.session_state.get("persisted_reissue_amount_cr", default_reissue_amount)
            )
        ...
        st.session_state["persisted_reissue_amount_cr"] = reissue_amount
...
def navigate_to_page(page_key: str) -> None:
    requested_page = HOMEPAGE_LINK_MAP.get(page_key)
    if requested_page:
...
def get_latest_app_update_label(_source_data_version: str) -> str:
    gsec_df = load_gsec_history_dataframe()
    if gsec_df.empty or "date" not in gsec_df.columns:
        return "-"
    latest_date = pd.to_datetime(gsec_df["date"], errors="coerce").dropna().max()
    if pd.isna(latest_date):
        return "-"
    return latest_date.strftime("%d %b %Y")


def render_data_updated_line() -> None:
    update_label = get_latest_app_update_label(get_source_data_version())
    st.markdown(
        f"<p style='margin-top:-0.2rem; margin-bottom:0.6rem; color:#60748a; font-size:0.92rem; font-style:italic;'>Data updated till {update_label}</p>",
        unsafe_allow_html=True,
    )


def render_homepage() -> None:
    st.title("Market Loan Analytics")
    st.markdown(
    ...
    st.markdown(
        ...
        .homepage-start div.stButton {
            margin: 0;
            display: inline-block;
        }
        .homepage-start div.stButton > button {
        ...
        .homepage-start div.stButton > button:hover {
            color: #184a86;
            text-decoration: underline;
        }
        .homepage-start div.stButton > button:focus {
            box-shadow: none;
        }
    ...
    render_data_updated_line()
    contents_col, glossary_col = st.columns([0.58, 0.42], vertical_alignment="top")

    homepage_links = [
        ("View borrowing trends", "assam", "Assam Dashboard"),
        ("Compare states", "statewise", "State-wise Comparison"),
        ("Evaluate reissue vs fresh borrowing", "reissuances", "Reissue Simulator"),
        ("Track market conditions", "rates", "Interest Rate Movements"),
    ]
    with contents_col:
        st.subheader("CONTENTS")
        for idx, (prefix, page_key, destination) in enumerate(homepage_links):
            line_col1, line_col2 = st.columns([0.03, 0.97])
            line_col1.markdown('<div class="homepage-bullet">&rarr;</div>', unsafe_allow_html=True)
            with line_col2:
                if page_key == "reissuances":
                    st.markdown(
                        '<div class="homepage-featured-note">Decision-support tool</div>',
                        unsafe_allow_html=True,
                    )
                    if st.button(
                        f"{prefix}: **{destination}**",
                        key=f"homepage_nav_{idx}",
                        type="tertiary",
                    ):
    ...
    if "homepage_glossary_open" not in st.session_state:
        st.session_state["homepage_glossary_open"] = False
    with glossary_col:
        st.subheader("GLOSSARY")
        glossary_label = (
            "- Click to collapse"
            if st.session_state["homepage_glossary_open"]
            else "+ Click to expand"
        )
        if st.button(glossary_label, key="homepage_glossary_toggle", type="tertiary"):
            st.session_state["homepage_glossary_open"] = not st.session_state["homepage_glossary_open"]
            st.rerun()

        if st.session_state["homepage_glossary_open"]:
            glossary_items = [
                ("SDL", "State Development Loan. A bond issued by a state government to raise market borrowings."),
                ("Market loan", "Borrowing raised by a state government through auctions in the securities market."),
                ("G-Sec", "Government Security, usually referring to sovereign bonds issued by the Government of India."),
                ("10Y G-Sec yield", "The market yield on the benchmark 10-year Government of India bond, used as a broad interest-rate benchmark."),
                ("Auction date", "The date on which the borrowing auction is conducted."),
                ("Issue date", "The date on which the security is issued and funds are received."),
                ("Maturity date", "The date on which the principal on the security is repaid."),
                ("ISIN", "International Securities Identification Number. A unique code used to identify a specific security."),
                ("Cut-off yield", "The highest accepted yield in an auction and a key indicator of borrowing cost."),
                ("Weighted average yield", "Average accepted yield in an auction, weighted by accepted amounts."),
                ("Bid cover", "Ratio of total bids received to the amount accepted. Higher values usually indicate stronger demand."),
                ("Tenor", "Time remaining to maturity of a security."),
                (
                    "Tenor bucket",
                    "Grouping of securities by maturity range such as T1-5 (1-5 years), T6-10 (6-10 years), T11-15 (11-15 years), T16-20 (16-20 years), and T20+ (more than 20 years).",
                ),
                ("Outstanding", "Securities that are still live and not yet repaid."),
                ("RBI", "Reserve Bank of India, the central bank of India and the source of auction-related borrowing data used in the app."),
                ("FBIL", "Financial Benchmarks India Pvt. Ltd., a source for secondary-market SDL prices and yields."),
                ("Investing.com", "Source used in the app for historical benchmark 10-year G-Sec market yield data."),
                ("Market price", "The price at which a security is trading in the secondary market."),
                ("YTM", "Yield to Maturity. The annualized return implied by the current market price if the bond is held to maturity."),
                ("Premium to par", "When market price is above 100, meaning the security trades above face value."),
                ("Discount to par", "When market price is below 100, meaning the security trades below face value."),
                ("Par", "When market price is exactly 100, equal to face value."),
                ("Reissue", "Issuing additional amount under an existing ISIN instead of creating a fresh security."),
                ("Fresh issue", "Borrowing through a new security or a new ISIN."),
                ("Cash proceeds", "Funds received from issuance or reissue, based on face amount and issue price."),
                ("Upfront gain / shortfall", "Immediate excess or shortfall of cash proceeds relative to face amount."),
                ("Repayment schedule", "The time profile of principal repayments due across future months and years."),
                ("Maturity-month liability", "Total principal repayment already due in the month when a selected SDL matures."),
            ]
            glossary_html = "".join(
                f"<p><strong>{term}:</strong> {definition}</p>"
                for term, definition in glossary_items
            )
            st.markdown(
                f"""
                <div class="homepage-glossary-card">
                    {glossary_html}
                </div>
                """,
                unsafe_allow_html=True,
...
def main() -> None:
    ...
    if selected_page == "Reissuances":
        st.markdown(
            '<div class="page-title-row"><h1>Reissue Scenario Analysis</h1>'
            '<span class="page-badge">Decision-support tool</span></div>',
            unsafe_allow_html=True,
        )
        render_data_updated_line()
        ...
        with left_col:
            ...
            if maturity_dates.notna().any():
                ...
                filter_col1, filter_col2 = st.columns([1.8, 1])
                default_tenor_range = get_persisted_reissue_tenor_range(slider_min, slider_max)
                if "reissue_tenor_range" not in st.session_state:
                    st.session_state["reissue_tenor_range"] = default_tenor_range
                selected_tenor_range = filter_col1.slider(
                    "Select tenor range (years)",
                    min_value=slider_min,
                    max_value=slider_max,
                    step=1,
                    key="reissue_tenor_range",
                )
                st.session_state["persisted_reissue_tenor_range"] = selected_tenor_range
                if "reissue_vintage_filter" not in st.session_state:
                    st.session_state["reissue_vintage_filter"] = st.session_state.get("persisted_reissue_vintage_filter", "1 year")
                selected_vintage = filter_col2.selectbox(
                    "SDL vintage",
                    ["1 year", "2 years", "Earlier", "All"],
                    key="reissue_vintage_filter",
                )
                st.session_state["persisted_reissue_vintage_filter"] = selected_vintage
                reissue_df = filter_reissue_candidates_by_tenor(reissue_df, selected_tenor_range)
                reissue_df = filter_reissue_candidates_by_vintage(reissue_df, selected_vintage)
            selected_reissue_row = render_reissue_candidates_table(reissue_df)
            recommended_reissues_df = rank_reissue_recommendations(reissue_df, maturity_profile_df, threshold_cr=2000.0)
            render_reissue_recommendations(recommended_reissues_df, threshold_cr=2000.0)
        with right_col:
    ...
    if selected_page == "State-wise comparison":
        ...
        filter_col1, filter_col2, filter_col3 = st.columns([1, 1, 1.8])
        ...
        default_selected_months = [month for month in get_default_statewise_months() if month in month_labels]
        ...
        statewise_analytics = get_statewise_analysis(
            connection,
            selected_months=selected_months,
            current_fy=selected_current_fy,
            comparison_fy=selected_comparison_fy,
        )
        render_statewise_comparison_table(comparison_df, selected_months, analytics=statewise_analytics)
        return
    ...
    if selected_page == "Interest Rate Movements":
        ...
        window_choice = st.segmented_control(
            "Select window",
            options=["5Y", "1Y", "6M", "1M", "1W", "Custom"],
            default="1W",
            selection_mode="single",
        )
        selected_window = window_choice or "1W"
        custom_start = None
        custom_end = None
        ...
        with left_col:
            render_gsec_trend_chart(gsec_chart_df, selected_window)
        with right_col:
