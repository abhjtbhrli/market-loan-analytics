...
def get_statewise_analysis(
    ...
    tenor_yield_by_bucket: dict[str, pd.DataFrame] = {}
    tenor_bucket_order = ["T1-5", "T6-10", "T11-15", "T16-20", "T20+"]
    tenor_yield_source_df = filtered_df.dropna(subset=["cut_off_yield"]).copy()
    tenor_yield_source_df["tenor_group"] = tenor_yield_source_df["tenor_group"].astype("string").str.strip()
    available_tenor_buckets = [
        bucket for bucket in tenor_bucket_order if bucket in tenor_yield_source_df["tenor_group"].dropna().unique()
    ]
    for tenor_bucket in available_tenor_buckets:
        bucket_df = tenor_yield_source_df[tenor_yield_source_df["tenor_group"] == tenor_bucket].copy()
        tenor_yield_by_bucket[tenor_bucket] = _build_state_fy_metric_pivot(
            bucket_df,
            metric_column="cut_off_yield",
            aggregator="mean",
            selected_current_fy=selected_current_fy,
    ...
    return {
        ...
        "tenor_yield_by_bucket": tenor_yield_by_bucket,
        "tenor_yield_buckets": available_tenor_buckets,
